...
class KinesisPublisher implements EventListener {
  ...
  private ListenableFuture<Boolean> publishSync(
    ...
    try {
      ...
      logger.atSevere().withCause(futureException).log(
          "KINESIS PRODUCER - Failed completing future to publish event %s [PK: %s]",
  ...
  private ListenableFuture<Boolean> publishAsync(
    ...
    try {
      ...
      Function<UserRecordResult, Boolean> userToPublish =
          new Function<UserRecordResult, Boolean>() {
            @Override
            public Boolean apply(UserRecordResult userRecordResult) {
              return userRecordResult != null && userRecordResult.isSuccessful();
            }
          };

      return Futures.transform(publishF, userToPublish, MoreExecutors.directExecutor());
    } catch (Exception e) {
      logger.atSevere().withCause(e).log(
